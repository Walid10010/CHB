...
def run_cohesion_on_arrays(
    ...
    *,
    ...
    disable_t3: bool = False,
    run_t3_on_low_dim: bool = False,
    compute_kmeans_scores: bool = True,
    ...
    if compute_kmeans_scores:
        kmeans_scores = compute_kmeans_ref_scores(X_proc, y)
    else:
        kmeans_scores = {"has_ground_truth": False,
                         "skipped": "compute_kmeans_scores=False (convenience score only)"}
...
def _combined_cli_main():
    ...
    if len(sys.argv) < 2 or sys.argv[1] in ('-h', '--help'):
        print("""Usage (console script `chb`, or `python -m chb`):
  chb both       [shared-args...]     # full CHB run on one dataset (fingerprint + regime)
  chb batch                           # run all datasets in configured dirs
  chb chb        --report PATH        # annotate existing combined JSON(s) with the CHB block
  chb cohesion   [cohesion-args...]   # cohesion/topology block only
  chb separation [separation-args...] # separation block only
